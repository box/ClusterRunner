...
class BuildStore(object):
    ...
    def get(cls, build_id: int) -> Optional[Build]:
        ...
        if build is None:
            cls._logger.info('Requested build with id {} was not found in cache, checking database.'.format(build_id))
            build = cls._reconstruct_build(build_id)
            if build is not None:
                cls._cached_builds_by_id[build_id] = build
                cls._logger.notice('Build with id {} was added to cache.'.format(build_id))

        return build

    @classmethod
    def _reconstruct_build(cls, build_id):
        build_request_query = cls._session.query(BuildRequestSchema) \
                                          .filter(BuildRequestSchema.build_id == build_id) \
                                          .first()
        # We can assume no build exists with this build_id
        if build_request_query is None:
            return None

        build_request = BuildRequest({
            'type': build_request_query.type,
            'url': build_request_query.url,
            'branch': build_request_query.branch,
            'job_name': build_request_query.job_name
        })
        build = Build(build_request)
        build._build_id = build_id
        build.generate_project_type()

        build_meta = cls._session.query(BuildMetaSchema) \
                                 .filter(BuildMetaSchema.build_id == build_id) \
                                 .first()

        build._artifacts_tar_file = build_meta.artifacts_tar_file
        build._artifacts_zip_file = build_meta.artifacts_zip_file
        build._error_message = build_meta.error_message
        build._postbuild_tasks_are_finished = build_meta.postbuild_tasks_are_finished
        build.setup_failures = build_meta.setup_failures
        build._timing_file_path = build_meta.timing_file_path

        fsm_timestamps = cls._session.query(BuildFsmSchema) \
                                     .filter(BuildFsmSchema.build_id == build_id) \
                                     .first()

        # Set the state machine timestamps
        ...
        build._state_machine._fsm.current = fsm_timestamps.state

        build_artifact_query = cls._session.query(BuildArtifactSchema) \
                                           .filter(BuildArtifactSchema.build_id == build_id) \
                                           .first()

        # Create the build_artifact
        build_artifact = BuildArtifact(build_artifact_query.build_artifact_dir)

        # Attribute of build_artifact
        failed_artifact_directories = cls._session.query(FailedArtifactDirectoriesSchema) \
                                                  .filter(FailedArtifactDirectoriesSchema.build_id == build_id) \
                                                  .all()
        directories = []
        for directory in failed_artifact_directories:
        ...
        # Place subjobs into correct queues
    ...
    def add(cls, build: Build):
        ...
        build._build_id = build_id
        cls._cached_builds_by_id[build_id] = build
        # cls._cached_builds_by_id[build.build_id()] = build
    ...
    def size(cls) -> Tuple[int, int]:
        ...
        """
        total_len = cls._session.query(func.count('*')).select_from(BuildSchema).scalar()
    ...
    def _update_build(cls, build: Build) -> int:
        ...
        # Build status
        build_schema = cls._session.query(BuildSchema).filter(BuildSchema.build_id == build_id).first()

        if build_schema is None:
            raise ItemNotFoundError('Unable to find in database build with id: {}.'.format(build_id))

        build_schema.completed = build._status() == BuildState.FINISHED
        build_schema.prepared = build._status() != BuildState.PREPARING

        # Build meta
        build_meta = cls._session.query(BuildMetaSchema) \
                         .filter(BuildMetaSchema.build_id == build_id) \
                         .first()
        build_meta.artifacts_tar_file = build._artifacts_tar_file
        build_meta.artifacts_zip_file = build._artifacts_zip_file
        build_meta.error_message = build._error_message
        build_meta.postbuild_tasks_are_finished = build._postbuild_tasks_are_finished
        build_meta.setup_failures = build.setup_failures
        build_meta.timing_file_path = build._timing_file_path
        ...
        if build._build_artifact is not None:
            ...
            cls._session.query(FailedArtifactDirectoriesSchema) \
               .filter(FailedArtifactDirectoriesSchema.build_id == build_id) \
               .delete(synchronize_session=False)  # True?
            cls._session.commit()
