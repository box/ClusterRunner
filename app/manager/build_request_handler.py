...
class BuildRequestHandler(object):
    ...
    This class manages two critical Queues in ClusterRunner: request_queue and builds_waiting_for_workers.

    The request_queue is the queue of non-prepared Build instances that the BuildRequestHandler has
    yet to prepare. This queue is populated by the ClusterManager instance.

    The builds_waiting_for_workers queue is the queue of prepared Build instances that the
    BuildRequestHandler has completed build preparation for, and is waiting for the WorkerAllocator (a separate
    ...
    def _build_preparation_loop(self):
        ...
        and puts that build onto the self.builds_waiting_for_workers queue.
        """
        while True:
        ...
        with project_lock:
            ...
            try:
                ...
                if not build.is_stopped:
                    ...
                    # If the atomizer found no work to do, perform build cleanup and skip the worker allocation.
                    if len(build.get_subjobs()) == 0:
                        self._logger.info('Build {} has no work to perform and is exiting.', build.build_id())
                        build.finish()
                    # If there is work to be done, this build must queue to be allocated workers.
                    else:
                        self._logger.info('Build {} is waiting for workers.', build.build_id())
                        self._scheduler_pool.add_build_waiting_for_workers(build)
