...
from app.manager.atom import AtomState
...
class Subjob(object):
    def __init__(self, build_id, subjob_id, project_type, job_config, atoms):
        ...
        self.worker = None  # The worker that had been assigned this subjob. Is None if not started.
    ...
    def mark_in_progress(self, worker):
        """
        Mark the subjob IN_PROGRESS, which marks the state of all the atoms of the subjob IN_PROGRESS.

        :param worker: the worker node that has been assigned this subjob.
        :type worker: Worker
        """
    ...
    def api_representation(self):
        ...
        return {
            ...
            'worker': self.worker.url if self.worker else None,
        }
    ...
    def get_atoms(self, offset: int=None, limit: int=None) -> List['Atom']:
        ...
        :rtype: list[app.manager.atom.Atom]
        """
    ...
    def add_timings(self, timings):
        """
        Add timing data for this subjob's atoms, collected from a worker
        :param timings:
