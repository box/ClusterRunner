...
class TimeBasedAtomGrouper2(object):

    MINIMUM_SUBJOB_SECONDS = 20  # todo: put into config
    ...
    def groupings(self):
        ...
        # 2). Sort them by time
        atoms_sorted_by_time = sorted(self._atoms, key=lambda a: a.expected_time, reverse=True)

        # 3). Group them! Iterate through the list in order creating subjobs of no less than MINIMUM_SUBJOB_SECONDS.
        subjobs = []
        next_subjob = []
        subjob_estimated_time = 0

        for atom in atoms_sorted_by_time:
            next_subjob.append(atom)
            subjob_estimated_time += atom.expected_time

            if subjob_estimated_time > self.MINIMUM_SUBJOB_SECONDS:
                subjobs.append(next_subjob)
                next_subjob = []
                subjob_estimated_time = 0
    ...
    def _coalesce_new_atoms_with_historic_times(self, new_atoms, old_atoms_with_times, project_directory):
        ...
        for new_atom in new_atoms:
            ...
            new_atom.expected_time = atom_time
    ...
    def _group_atoms_into_sized_buckets(self, sorted_atom_time_dict, target_group_time, max_groups_to_create):
        ...
        :type sorted_atom_time_dict: OrderedDict[Atom, float]
