...
class TestHttpTimeout(BaseFunctionalTestCase):
    ...
    @genty_dataset(
        on_first_request=([],),
        on_second_request=(['{"worker": {"is_alive": true}}'],),
    )
    def test_unresponsive_worker_does_not_hang_manager(self, responses):
        # Start the manager and an unresponsive worker.
        manager = self.cluster.start_manager(default_http_timeout=1)
        unresponsive_worker = UnresponsiveServer().start(
            port=self.UNRESPONSIVE_SLAVE_PORT,
            responses=deque(responses))
        unresponsive_worker_id = manager.connect_worker(
            'http://localhost:{}'.format(self.UNRESPONSIVE_SLAVE_PORT),
            num_executors=1)

        self.addCleanup(unresponsive_worker.stop)

        # Start a build which will be cause an attempt to allocate the unresponsive worker.
        ...
        self.assertTrue(manager.block_until_worker_offline(unresponsive_worker_id, timeout=10),
                        'Unresponsive worker should be marked offline.')

        # First worker should now be marked offline. Connect a real worker to finish the build.
        self.cluster.start_workers(num_workers=1, start_port=self.NORMAL_SLAVE_PORT)
...
class UnresponsiveServer:
    """
    Server to emulate an unresponsive worker. This can optionally can return a sequence of
