...
class TestConsoleOutput(BaseFunctionalTestCase):
    ...
    def test_logs_are_still_available_after_worker_goes_offline(self):
        ...
        # Bring down the single worker and assert that console output for the build is still available.
        self.cluster.kill_workers()

        console_output_1 = manager.get_console_output(build_id=build_id, subjob_id=0, atom_id=0)
        self.assertEqual(
            console_output_1['content'].strip(),
            'Doing subjob 1.'
        )
        console_output_2 = manager.get_console_output(build_id=build_id, subjob_id=1, atom_id=0)
        self.assertEqual(
