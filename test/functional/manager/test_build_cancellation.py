...
class TestBuildCancellation(BaseFunctionalTestCase):
    def test_build_cancellation_while_building(self):
        ...
        workers = self.cluster.start_workers(1, num_executors_per_worker=1, start_port=43001)
        ...
        self.assertTrue(manager.block_until_build_canceled(build_id, timeout=30),
        ...
        # Make sure that worker becomes idle before the total job execution time (90 sec).
        ...
        self.assertTrue(workers[0].block_until_idle(timeout=10),
                        'The worker should become idle within the timeout.')

    def test_build_cancellation_while_atomizing(self):
        self._remove_file(ATOMIZER_PID_FILE)
        manager = self.cluster.start_manager()
        # Start a worker even though its not needed as build is canceled during atomization
        workers = self.cluster.start_workers(1, num_executors_per_worker=1, start_port=43001)
        project_dir = tempfile.TemporaryDirectory()
        build_resp = manager.post_new_build({
            'type': 'directory',
        ...
        build_id = build_resp['build_id']
        self.assertTrue(manager.block_until_build_has_status(build_id, [BuildState.QUEUED], timeout=30),
        ...
        self.assert_build_status_contains_expected_data(build_id, {'status': BuildState.CANCELED})
        # Make sure that worker is in idle state
        self.assertTrue(workers[0].is_worker_idle(),
