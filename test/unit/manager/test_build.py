...
from app.manager.atom import Atom, AtomState
from app.manager.atomizer import Atomizer, AtomizerError
from app.manager.build import Build, BuildStatus, BuildProjectError
from app.manager.build_fsm import BuildState
...
from app.manager.worker import Worker, DeadWorkerError, WorkerCommunicationError, WorkerMarkedForShutdownError
from app.manager.subjob import Subjob
from app.manager.subjob_calculator import compute_subjobs_for_build
...
class TestBuild(BaseUnitTestCase):
    ...
    _FAKE_SLAVE_URL = 'my.favorite.worker.com:40001'
    ...
    def setUp(self):
        ...
        self.patch('app.manager.build.BuildArtifact.__new__')  # patch __new__ to mock instances but keep static methods
        self.mock_util = self.patch('app.manager.build.app.util')  # stub out util - it often interacts with the fs
        self.mock_open = self.patch('app.manager.build.open', autospec=False, create=True)
    ...
    def test_allocate_worker_calls_worker_setup(self):
        mock_worker = self._create_mock_worker()
        ...
        mock_worker.setup.assert_called_once_with(build, executor_start_index=0)

    def test_build_doesnt_use_more_than_max_executors(self):
        mock_workers = [self._create_mock_worker(num_executors=5) for _ in range(3)]  # 15 total available executors
        ...
        for mock_worker in mock_workers:
            scheduler.allocate_worker(mock_worker)
            scheduler.begin_subjob_executions_on_worker(mock_worker)
    ...
    def test_build_doesnt_use_more_than_max_executors_per_worker(self):
        mock_workers = [self._create_mock_worker(num_executors=5) for _ in range(3)]
        max_executors_per_worker = 2
        job_config = self._create_job_config(max_executors_per_worker=max_executors_per_worker)
        ...
        # Even though each worker has 5 executors, we should only start subjobs on 2 of those executors per worker.
        ...
        self.assertEqual(
            ...
            expected_subjob_execution_calls,
            'Build should start executing as many subjobs per worker as its max_executors_per_worker setting.')
    ...
    def test_build_status_returns_building_after_setup_has_started(self):
        ...
        self.assertEqual(build._status(), BuildStatus.BUILDING,
                         'Build status should be BUILDING after setup has started on workers.')
    ...
    def test_build_status_returns_building_after_setup_is_complete_and_subjobs_are_executing(self):
        ...
        self.assertEqual(build._status(), BuildStatus.BUILDING,
                         'Build status should be BUILDING after subjobs have started executing on workers.')

    def test_build_status_returns_finished_after_all_subjobs_complete_and_workers_finished(self):
        build = self._create_test_build(BuildStatus.BUILDING)
    ...
    def test_need_more_workers(
            self,
            max_executors_for_build,
            build_should_need_more_workers,
            num_subjobs
    ):
        ...
        mock_worker = self._create_mock_worker(num_executors=5)
        scheduler.allocate_worker(worker=mock_worker)

        self.assertEqual(scheduler.needs_more_workers(), build_should_need_more_workers,
                         'If and only if the maximum number of executors is allocated we should not need more workers.')
    ...
    def test_teardown_called_on_worker_when_no_subjobs_remain(self):
        mock_worker = self._create_mock_worker(num_executors=1)
        self._create_test_build(BuildStatus.FINISHED, num_subjobs=1, workers=[mock_worker])

        mock_worker.teardown.assert_called_with()

    def test_teardown_called_on_all_workers_when_no_subjobs_remain(self):
        mock_workers = [
            self._create_mock_worker(num_executors=5),
            self._create_mock_worker(num_executors=4),
            self._create_mock_worker(num_executors=3),
        ]
        self._create_test_build(BuildStatus.FINISHED, num_subjobs=20, workers=mock_workers)

        for mock_worker in mock_workers:
            mock_worker.teardown.assert_called_with()

    @genty_dataset(DeadWorkerError, WorkerCommunicationError, WorkerMarkedForShutdownError)
    def test_teardown_called_on_worker_when_start_subjob_raises(self, exception_cls):
        mock_worker = self._create_mock_worker(num_executors=5)
        mock_worker.start_subjob.side_effect = exception_cls

        self._create_test_build(BuildStatus.BUILDING, num_subjobs=30, workers=[mock_worker])

        mock_worker.teardown.assert_called_with()

    def test_teardown_called_on_worker_when_worker_is_not_alive(self):
        mock_worker = self._create_mock_worker(num_executors=5)
        mock_worker.start_subjob.side_effect = WorkerMarkedForShutdownError
    ...
    def test_cancel_prevents_further_subjob_starts_and_sets_canceled(self):  # dev: this is flaky now
        mock_worker = self._create_mock_worker(num_executors=5)
        build = self._create_test_build(BuildStatus.BUILDING, num_subjobs=30, workers=[mock_worker])

        self.assertEqual(mock_worker.start_subjob.call_count, 5, 'Worker should only have had as many subjobs started '
        ...
        self.assertEqual(mock_worker.start_subjob.call_count, 5, 'A canceled build should not have any more subjobs '
                                                                'started after it has been canceled.')

    def test_cancel_is_a_noop_if_build_is_already_finished(self):
        mock_worker = self._create_mock_worker()
        build = self._create_test_build(BuildStatus.FINISHED, workers=[mock_worker])
        num_worker_calls_before_cancel = len(mock_worker.method_calls)

        build.cancel()

        self.assertEqual(build._status(), BuildStatus.FINISHED,
                         'Canceling a finished build should not change its state.')
        self.assertEqual(len(mock_worker.method_calls), num_worker_calls_before_cancel,
                         'Canceling a finished build should not cause any further calls to worker.')
    ...
    def test_execute_next_subjob_with_no_more_subjobs_should_not_teardown_same_worker_twice(self):
        mock_worker = self._create_mock_worker()
        build = self._create_test_build(BuildStatus.BUILDING, workers=[mock_worker])
        scheduler = self.scheduler_pool.get(build)
        self._finish_test_build(build, assert_postbuild_tasks_complete=False)
        ...
        self.assertEqual(mock_worker.teardown.call_count, 1, "Teardown should only be called once")

    def test_worker_is_fully_allocated_when_max_executors_per_worker_is_not_set(self):
        mock_worker = self._create_mock_worker(num_executors=10)
        job_config = self._create_job_config(max_executors_per_worker=float('inf'))
        self._create_test_build(BuildStatus.BUILDING, job_config=job_config, workers=[mock_worker])

        self.assertEqual(mock_worker.claim_executor.call_count, 10, 'Claim executor should be called once for each '
                                                                   'of the worker executors.')

    def test_worker_is_only_allocated_up_to_max_executors_per_worker_setting(self):
        mock_worker = self._create_mock_worker(num_executors=10)
        job_config = self._create_job_config(max_executors_per_worker=5)
        self._create_test_build(BuildStatus.BUILDING, job_config=job_config, workers=[mock_worker])

        self.assertEqual(mock_worker.claim_executor.call_count, 5, 'Claim executor should be called '
                                                                  'max_executors_per_worker times.')

    def test_generate_project_type_raises_error_if_failed_to_generate_project(self):
        build = self._create_test_build()
        self.patch('app.manager.build.util.create_project_type').return_value = None
    ...
    def test_allocating_worker_to_build_sets_building_timestamp_only_on_first_worker_allocation(self):
        mock_worker1 = self._create_mock_worker()
        mock_worker2 = self._create_mock_worker()
        build = self._create_test_build(BuildStatus.PREPARED)
        scheduler = self.scheduler_pool.get(build)

        self.assertIsNone(self._get_build_state_timestamp(build, BuildState.BUILDING),
                          '"building" timestamp should not be set until worker allocated.')

        scheduler.allocate_worker(worker=mock_worker1)
        building_timestamp1 = self._get_build_state_timestamp(build, BuildState.BUILDING)
        scheduler.allocate_worker(worker=mock_worker2)
        building_timestamp2 = self._get_build_state_timestamp(build, BuildState.BUILDING)
        self.assertIsNotNone(building_timestamp1, '"building" timestamp should be set after first worker allocated.')
        self.assertEqual(building_timestamp1, building_timestamp2,
                         '"building" timestamp should not change upon further worker allocation.')
    ...
    def test_delete_temporary_build_artifact_files_skips_results_tarball(self):
        ...
        mock_shutil = self.patch('app.manager.build.shutil')
    ...
    def _create_test_build(
        ...
        self.patch('app.manager.build.util.create_project_type').return_value = mock_project_type
        build.generate_project_type()
        ...
        # BUILDING: Allocate a worker and begin subjob executions on that worker.
        workers = workers or [self._create_mock_worker()]
        scheduler = self.scheduler_pool.get(build)
        for worker in workers:
            scheduler.allocate_worker(worker=worker)
            scheduler.begin_subjob_executions_on_worker(worker=worker)
    ...
    def _create_job_config(
        ...
        max_executors_per_worker=_FAKE_MAX_EXECUTORS_PER_SLAVE,
    ):
        atomizer = Atomizer([{'FAKE': 'fake atomizer command'}])
        return JobConfig('', '', '', '', atomizer, max_executors, max_executors_per_worker)

    def _create_mock_project_type(self):
        return MagicMock(spec_set=ProjectType())

    def _create_mock_worker(self, num_executors=5):
        """
        :type num_executors: int
        :rtype: Worker | MagicMock
        """
        worker_spec = Worker('', 0)  # constructor values don't matter since this is just a spec object
        mock_worker = MagicMock(spec_set=worker_spec, url=self._FAKE_SLAVE_URL, num_executors=num_executors)

        counter = Counter()
        mock_worker.claim_executor.side_effect = counter.increment
        mock_worker.free_executor.side_effect = counter.decrement
    ...
    def _patch_compute_subjob_for_build(self, subjobs, compute_subjob_raises_exception=False):
        ...
        """
        mock_compute_subjobs_for_build = self.patch('app.manager.build.compute_subjobs_for_build')
    ...
    def _finish_test_build(self, build, assert_postbuild_tasks_complete=True):
        ...
        while build_has_running_subjobs:
            ...
            # copy allocated_workers list since workers may get deallocated during loop
            workers_allocated = build_scheduler._workers_allocated.copy()
            for mock_worker in workers_allocated:
                self.assertIsInstance(mock_worker, Mock,
                                      '_finish_test_build() can only be used on builds with mock workers.')

                for subjob in self._get_in_progress_subjobs_for_mock_worker(mock_worker):
                    build_has_running_subjobs = True
                    build.complete_subjob(subjob.subjob_id())
                    build_scheduler.execute_next_subjob_or_free_executor(mock_worker)
    ...
    def _get_in_progress_subjobs_for_mock_worker(self, mock_worker):
        return [
            start_subjob_args[0]
            for start_subjob_args, _ in mock_worker.start_subjob.call_args_list
