...
class TestRemoteManagerService(BaseUnitTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_start_and_block_until_up_raises_exception_if_process_fails_to_start(self):
        self._mock_shell_exec_command({
            'nohup some_path manager --port 43000 &': "\n",
            'ps ax | grep \'[s]ome_path\'': "\n",
        })
        remote_manager_service = RemoteManagerService('some_host', 'some_username', 'some_path')
        with self.assertRaisesRegex(SystemExit, '1'):
            remote_manager_service.start_and_block_until_up(43000, 5)
    ...
    def test_start_and_block_until_up_raises_exception_if_process_starts_by_service_doesnt_respond(self):
        ...
        self.patch('app.deployment.remote_manager_service.ServiceRunner').return_value.is_up.return_value = False
        remote_manager_service = RemoteManagerService('some_host', 'some_username', 'some_path')
        with self.assertRaisesRegex(SystemExit, '1'):
            remote_manager_service.start_and_block_until_up(43000, 5)

    def test_start_and_block_until_up_doesnt_raise_exception_if_manager_service_is_up(self):
        self._mock_shell_exec_command({
            'nohup some_path manager --port 43000 &': "\n",
            'ps ax | grep \'[s]ome_path\'': "\nsome_path\n",
        })
        self.patch('app.deployment.remote_manager_service.ServiceRunner').return_value.is_up.return_value = True
        remote_manager_service = RemoteManagerService('some_host', 'some_username', 'some_path')
        remote_manager_service.start_and_block_until_up(43000, 5)
    ...
    def test_is_process_running_returns_false_if_only_empty_output(self):
        ...
        remote_manager_service = RemoteManagerService('some_host', 'some_username', 'some_path')
        self.assertFalse(remote_manager_service._is_process_running('some_command'))

    def test_is_process_running_returns_true_if_found_non_empty_output(self):
        self._mock_shell_exec_command({'ps ax | grep \'[s]ome_command\'': "\nrealoutput\n"})
        remote_manager_service = RemoteManagerService('some_host', 'some_username', 'some_path')
        self.assertTrue(remote_manager_service._is_process_running('some_command'))
